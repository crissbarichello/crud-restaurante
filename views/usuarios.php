...
<div class="container">
    ...
    <?php
        ...
        $acaoForm = !isset($usuarioEditando) ? 'cadastrar' : 'atualizar';
    ?>
    <form method="POST" action="index.php?acao=<?= $acaoForm ?><?= !isset($usuarioEditando) ? '&id=' . !isset($usuarioEditando['id']) : '' ?>">
        <div>
            <label for="nome"><?= !isset($usuarioEditando) ? 'Editar usuário' : 'Novo usuário' ?></label>
            <input type="text" id="nome" name="nome" value="<?= htmlspecialchars($usuarioEditando['nome'] ?? '') ?>" required>
        </div>
        <div>
            <label for="email">Email</label>
            <input type="text" id="email" name="email" value="<?= htmlspecialchars($usuarioEditando['email'] ?? '') ?>" required>
        </div>
        <div>
            <label for="senha">Senha</label>
            <input type="password" id="senha" name="senha" value="<?= htmlspecialchars($usuarioEditando['senha'] ?? '') ?>" required>
        </div>
        <button type="submit">Salvar</button>
        <?php if (isset($usuarioEditando)): ?>
    ...
    <table>
        ...
        <tbody>
            <?php foreach (isset($usuarios) ? $usuarios : [] as $usuario): ?>
                <tr>
